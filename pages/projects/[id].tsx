...
const Project: NextPage = (): JSX.Element => {
  ...
  useEffect(() => {
    ...
    const getStories = async () => {
      const getFetchStringFromState = (state: string) => {
      ...
      const getStoriesForFetchString = (apiKey: string, projectId: string) => (
        fetchString: string
      ) =>
        axios.get(
      ...
      const formatResult = (acc: Record<string, Story[]>, stories: Story[], idx: number) => ({
        ...acc,
        [states[idx]]: stories
      });
      ...
      const stories = (
        await Promise.all(
          states.map(getFetchStringFromState).map(getStoriesForFetchString(apiKey, id))
        )
      )
        .map(({ data }: AxiosResponse): Story[] => data)
        .reduce(formatResult, {} as Record<string, Story[]>);
