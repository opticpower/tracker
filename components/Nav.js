import { Text, Row, Col, Spacer, Select } from '@geist-ui/react';
import { Sun, Moon } from '@geist-ui/react-icons';

const opticLogo = '/images/OpticPower.png';

const Nav = ({ useLight, setUseLight }) => {
  return (
    <Row className="nav">
      <style jxs>
        {`
      ...
      <Col align="right">
        <Select placeholder={useLight ? 'Light Mode' : 'Dark Mode'}>
          <Select.Option value="Dark Mode" onClick={() => setUseLight(false)}>
            <Moon size={16} /> <Spacer inline x={0.35} />
            Dark Mode
          </Select.Option>
          <Select.Option value="Light Mode" onClick={() => setUseLight(true)}>
